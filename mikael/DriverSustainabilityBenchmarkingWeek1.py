...
func = lambda x: (-1601 * np.exp(-800 * (x - 0.4)**2) + (-1600 * x + 640.0)**2 * np.exp(-800 * (x - 0.4)**2) - 20.25 * np.exp(-40 * (x - 0.8)**2) + 0.25 * (-80 * x + 64.0)**2 * np.exp(-40 * (x - 0.8)**2))
...
plt.title(f'Group: <Your Group ID>, Iter: {iter}, Time: {CPUtime:.4f} s, DOF: {DOF}, CO2e={CO2eq:.4e} kg CO2')
plt.legend(['Exact', 'AMR'], loc='northeast')

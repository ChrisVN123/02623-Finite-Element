...
print(h_arr)
plot(h_arr,error_arr,title = "Mean element length vs max approximate error", xlabel = "log(mean element length)", ylabel = "log(max error)", primary=false, xaxis = (:log10, [10^-5, :auto]), yaxis = (:log10, [10^-5, :auto]))

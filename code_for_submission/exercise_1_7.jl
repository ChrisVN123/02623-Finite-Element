...
err_DOF_arr = []
for DOF in DOF_arr
    x_DOF = LinRange(0, L, DOF)
    _, _, u_DOF = BVP1Drhs(L, c, d, x_DOF, f)

    u_DOF_arr = u_hat.(x_arr, [u_DOF], [x_DOF])
    err_DOF = sqrt(sum(
        (u_arr .- u_DOF_arr).^2 .* (1/1_000)
    ))
    push!(err_DOF_arr, err_DOF)
end 

plot(DOF_arr, err_DOF_arr, title="err vs DOF")
savefig("exercise_1_7_f_DOF_err.png")